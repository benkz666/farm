...
任务按服务器 `time.Local` 的自然日（本地 00:00）重置；每名玩家每天固定有 4 条任务，不随农场 demo 时间档缩放。任务首次读取时按 `(uid, YYYYMMDD)` 初始化，`TaskList` 同时返回下一次本地 00:00 的 `reset_at`。
...
| 完成一次收获 | 1 次 | 30 | 0 |
| 拜访一次好友农场 | 1 次 | 40 | 0 |
| 每日登录 | 1 次（初始化即完成） | 100 | 0 |

播种、收获与拜访任务的进度由玩法动作产生的事件驱动累加，而不是由任务系统主动轮询；每日登录任务则在初始化时完成。
...
每个服务器本地自然日最多可得 190 金币 / 0 经验。固定任务集规模小，奖励仅作为新手日常补贴，不影响农场昼夜 demo 的时间缩放。
