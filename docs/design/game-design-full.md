...
>
> 上游依据：[2009-classic-qq-farm-rules.md](../2009-classic-qq-farm-rules.md) —— 2009 年经典网页 QQ 农场的玩法考据。
>
...
**缩放因子必须在播种那一刻就折算进地块数据，而不是在每次读取时才乘。**

地块记录的是「本季生长时长（已缩放）」这个绝对值，因此地块是自描述的。若改为读取时折算，则修改配置会让所有在途作物的成熟时刻瞬间跳变或倒退。

同一农场内不同时期播种的作物可以带有不同的缩放结果，互不影响。

### 3.4 逻辑日

逻辑日是每日任务与每日上限的重置边界。它**跟随时间档缩放**，但设有下限：
...
设置下限的原因：若不设下限，`bench` 档下逻辑日只有 24 秒，每日上限和日常任务会被高频重置而失去约束意义。5 分钟同时保证了 `demo` 档下可以现场演示一次完整的每日重置。
...
进入成熟状态后，若超过 **3 倍本季生长时长**仍未收获，地块转为枯萎。

枯萎的后果：**整株作废**。本季产量归零，多季作物的剩余季数一并作废，必须清理后重新播种。

选择整株作废而非仅作废当季的理由：规则更简单，且让"及时收菜"成为贯穿全程的核心压力，与偷菜机制形成合力。3 倍本季时长的宽限期已相当宽松（白萝卜 30 缩放小时）。
...
任务进度由玩法动作产生的事件驱动累加，而不是由任务系统主动轮询。这让新增任务类型不需要改动玩法代码。

任务奖励**不直接发放，而是以邮件附件形式投递**，玩家自行领取。这样任务与邮件形成一条完整链路，同时把"领取"这个动作变成一个需要幂等保证的点。

### 14.2 奖励量级校验

每逻辑日最多可得 750 金币 / 75 经验（抽到奖励最高的三条）。验算确认其占日均产出的比例始终在合理区间：

| 配置 | 日均金币产出 | 任务占比 | 日均经验产出 | 任务占比 |
| --- | ---: | ---: | ---: | ---: |
| 6 地 × 白萝卜 | 2,117 | 35.4% | 438 | 17.1% |
| 6 地 × 南瓜 | 2,782 | 27.0% | 278 | 27.0% |
| 18 地 × 菠萝 | 11,284 | 6.6% | 582 | 12.9% |

占比随规模下降：对新手是有效补贴，对后期不构成主要收入。验算设有 50% 的上限断言。
...
**解锁条件**：首次收获某作物即永久解锁该条。偷到的果实不算解锁——必须是自己种出来的。

**收录范围**：26 种普通作物 + 3 种隐藏作物，共 29 条。

**图鉴条目内容**：作物名、解锁等级、种子价、季数、各季时长、产量、果实单价、收获经验、生长阶段示意。已解锁显示全部信息，未解锁只显示轮廓与解锁等级。

**里程碑奖励** 收集度达到 25% / 50% / 75% / 100%（即 8 / 15 / 22 / 29 条）时，通过邮件发放 1000 / 3000 / 8000 / 20000 金币。

图鉴是纯读取的数据，不参与任何玩法计算。它的作用是给收集行为一个正反馈，同时让隐藏作物有展示的地方。
