...
```text
Idle --发起--> Reserved --裁决成功--> Settled --> Idle
                       --裁决失败--> RolledBack --> Idle
                       --5 秒超时--> RolledBack --> Idle
```
...
**铁律二：`req_id` 全局唯一且由发起方生成。** 主人 Actor 用一张 64 条的 LRU 对 `req_id` 去重，保证即使消息被重复投递（集群阶段 MQ 的 at-least-once 语义）也只裁决一次。

**铁律三：超时必须回滚。** 5 秒超时。这个值的推导：单进程下跨 Actor 往返是微秒级，集群下经过一次内部 RPC，P99 在 10 ms 内。5 秒是留给「目标 Actor 正在从 MySQL 冷加载」的余量。超时后回滚预占并返回 `ERR_TIMEOUT`。
