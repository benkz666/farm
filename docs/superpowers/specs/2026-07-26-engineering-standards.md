...
| 期 | 目标 | 主要内容 | 验收 |
| ---: | --- | --- | --- |
| **0** | 规范落地 | 本规范 + 期 1 规格；前端归入 `client/`；compose；仓库骨架 | 文档审过；目录就位（**已合入 main**） |
| **1** | 联通切片 | 注册/登录、WS Handshake、EnterFarm 快照；Redis+MySQL 轻量读写；`client` net 联调 | 见 `2026-07-26-phase1-login-farm-snapshot.md`（**已合入 main**） |
| **2** | 种植权威 | `advance` + 种植循环；Buy/Sell；客户端 online 意图（等 Rsp）；期末化肥/多季 | 见 `2026-07-26-phase2-planting-authority.md`（**已合入 main**） |
| **3** | 多人同步 + 好友拜访 | 登录页、删本地权威；好友（链接+uid）；房间、FarmDelta、seq；镜像；访客只读 | 见 `2026-07-26-phase3-room-sync-friends.md`（**已合入 main**） |
| **4** | 社交闭环 + 多人骨架 | 分片 Gateway/Farm、Kafka 跨农场；互助；偷菜+狗；可偷摘要；用户名搜索；薄任务/邮件/每日登录 | 见 `2026-07-27-phase4-social-loop.md`（规格已评审） |
| **5** | 压测与材料 | 机器人、容量拐点、答辩/上线材料 | 有数据 + 演示脚本 |

**默认后置（需要时再插入对应期）**：完整 CSV 双端生成、ABC 分档 write-behind、逻辑分片热迁移、图鉴里程碑、狗升级树、隐藏种子。

分期规格：期 1–3 已有文档；期 4 见上表链接；期 5 另开。
