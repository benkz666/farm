...
| **3** | 多人同步 + 好友拜访 | 登录页、删本地权威；好友（链接+uid）；房间、FarmDelta、seq；镜像；访客只读 | 见 `2026-07-26-phase3-room-sync-friends.md` |
| **4** | 社交闭环 | 互助浇水、偷菜+狗；任务/邮件做薄 | 策划重点场景可演示 |
| **5** | 压测与材料 | 机器人、单机拐点、答辩材料 | 有数据 + 演示脚本 |

**默认后置（需要时再插入对应期）**：完整 CSV 双端生成、ABC 分档 write-behind、1024 分片多实例、图鉴里程碑、狗升级、隐藏种子、**按用户名搜索加好友**。
