...
func TestApplyPlotActionMainPath(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			check: func(t *testing.T, got *Aggregate) {
				...
				if p.SeasonDuration != 10*gameconfig.HourMs(gameconfig.TimeProfileDemo) {
				...
				got.Plots[0].WeedNextWin = gameconfig.RiskWindowsPerSeason
				got.Plots[0].PestNextWin = gameconfig.RiskWindowsPerSeason
			},
		},
		{
			name:   "生长中首次补水",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case Harvest:
				now += 10 * gameconfig.HourMs(gameconfig.TimeProfileDemo)
			}
...
func TestApplyPlotActionIllegalStateDoesNotMutateAggregate(t *testing.T) {
	...
	if result.Err != errcode.PlotNotWasteland {
		t.Fatalf("Err = %d, want %d", result.Err, errcode.PlotNotWasteland)
	}
...
func TestApplyPlotActionFailureDoesNotExpireCrossReservation(t *testing.T) {
	...
	if result.Err != errcode.PlotNotTilled {
...
func TestApplyPlotActionPlantOnWastelandRejected(t *testing.T) {
	...
	if result.Err != errcode.PlotNotTilled {
		t.Fatalf("Err = %d, want %d", result.Err, errcode.PlotNotTilled)
	}
	if !reflect.DeepEqual(*agg, before) {
...
func TestApplyPlotActionMatureCareClearsNegativeStateWithoutChangingYield(t *testing.T) {
	for _, kind := range []PlotActionKind{Water, Weed, Pest} {
		t.Run(kind.String(), func(t *testing.T) {
			...
			switch kind {
			case Water:
				agg.Plots[0].LastWaterAt = actionNow - agg.Plots[0].SeasonDuration
			case Weed:
				agg.Plots[0].WeedSince = actionNow - 1
			case Pest:
				agg.Plots[0].PestSince = actionNow - 1
			}
			...
			got := agg.Plots[0]
			if got.State != StateMature || got.FinalYield != 16 {
				t.Fatalf("mature care changed state/yield: %#v", got)
			}
			if kind == Water && got.LastWaterAt != actionNow {
				t.Fatalf("LastWaterAt = %d, want %d", got.LastWaterAt, actionNow)
			}
			if kind == Water {
				again := agg.ApplyPlotAction(PlotAction{Kind: Water, PlotIndex: 0}, actionNow+got.SeasonDuration)
				if again.Err != errcode.AlreadyWatered {
					t.Fatalf("mature Water after cleanup Err = %d, want AlreadyWatered", again.Err)
				}
			}
			if kind == Weed && got.WeedSince != 0 {
				t.Fatalf("WeedSince = %d, want 0", got.WeedSince)
			}
			if kind == Pest && got.PestSince != 0 {
				t.Fatalf("PestSince = %d, want 0", got.PestSince)
			}
		})
	}
}
...
func TestApplyPlotActionClearGrowingFailsWithoutMutation(t *testing.T) {
	...
	if result.Err != errcode.PlotNotCleanable {
		t.Fatalf("Err = %d, want %d", result.Err, errcode.PlotNotCleanable)
	}
	if !reflect.DeepEqual(*agg, before) {
...
func TestPlotActionPermissionMatrixMatchesDesign(t *testing.T) {
	want := map[uint8]map[PlotActionKind]bool{
		...
		StateMature:    {Water: true, Weed: true, Pest: true, Harvest: true, Steal: true},
...
func TestSuccessfulTillAndClearAwardEligibleHiddenSeeds(t *testing.T) {
	...
	agg.RecalcLevel()
	if result := agg.ApplyPlotAction(PlotAction{Kind: Till, PlotIndex: 0}, actionNow); result.Err != errcode.OK {
	...
	if result := agg.ApplyPlotAction(PlotAction{Kind: Clear, PlotIndex: 0}, actionNow); result.Err != errcode.OK {
...
func TestApplyPlotActionUsesAuthoritativeTimeProfile(t *testing.T) {
	...
	result := agg.ApplyPlotAction(PlotAction{
		...
		PlotIndex:   0,
		Arg:         10,
		TimeProfile: gameconfig.TimeProfileAuthentic,
	}, actionNow)
	if result.Err != errcode.OK {
		t.Fatalf("Err = %d, want OK", result.Err)
	}
...
func TestFertilizerUsesHotSwitchedCurrentSeasonTimeProfile(t *testing.T) {
	agg := NewAggregate(1, "alice")
	agg.Items[FertilizerItem(1)] = 1
	demoDuration := int64(10 * gameconfig.HourMs(gameconfig.TimeProfileDemo))
	agg.Plots[0] = Plot{
		State:          StateGrowing,
		CropID:         1,
		StageCount:     3,
	...
	authenticDuration := int64(10 * gameconfig.HourMs(gameconfig.TimeProfileAuthentic))
	...
	if got, want := agg.Plots[0].MatureAt, actionNow+authenticDuration-gameconfig.HourMs(gameconfig.TimeProfileAuthentic); got != want {
...
func TestAdvanceAllWithProfileReprofilesGrowingSeasonWithoutResettingProgress(t *testing.T) {
	...
	changes := agg.AdvanceAllWithProfile(now, gameconfig.TimeProfileAuthentic)
	...
	back := agg.AdvanceAllWithProfile(now, gameconfig.TimeProfileDemo)
...
func TestApplyPlotActionFertilizeRejectsSameStageWithoutMutation(t *testing.T) {
	...
	if result.Err != errcode.StageAlreadyFertilized {
		t.Fatalf("Err = %d, want %d", result.Err, errcode.StageAlreadyFertilized)
	}
	if !reflect.DeepEqual(*agg, before) {
