...
type FarmSnapshotJSON struct {
	...
	GuardDog      GuardDogSnapshot  `json:"guard_dog"`
}

// GuardDogSnapshot 是农场对访客可见的看门狗状态。
// 不暴露狗粮余量、拦截次数等主人私有数值；BowlEmptyAt 仅用于客户端切换本地表现。
type GuardDogSnapshot struct {
...
func (a *Aggregate) Snapshot() FarmSnapshotJSON {
	...
	return FarmSnapshotJSON{
		...
		Bag:           bag,
		Warehouse:     warehouse,
		GuardDog:      GuardDogSnapshotOf(a.Pet),
	}
}

// GuardDogSnapshotOf 只投影已启用且确实拥有的狗，避免历史脏数据渲染出不存在的狗。
func GuardDogSnapshotOf(pet PetState) GuardDogSnapshot {
	if !pet.HasDog(pet.ActiveDog) {
		return GuardDogSnapshot{}
	}
	return GuardDogSnapshot{
		ActiveDog:   pet.ActiveDog,
		BowlEmptyAt: pet.BowlEmptyAt,
	}
}

// VisitorSafeFarmSnapshot 去掉访客不应看到的个人经济字段（金币、经验、背包、仓库）。
// 保留昵称/等级/地块和看门狗公开状态，供拜访 HUD、偷菜判定与场景渲染使用。不修改入参。
