...
func TestVisitorSnapshotExposesOnlyGuardDogPresentationState(t *testing.T) {
	agg := NewAggregate(1, "alice")
	agg.Pet = PetState{
		ActiveDog:   DogShepherd,
		Owned:       1 << 1,
		BowlEmptyAt: 12_345,
		MsPerGram:   ShepherdMsPerGram,
		DogLevel:    [3]uint8{1, 4, 0},
		Intercepts:  [3]uint16{3, 88, 0},
	}

	visitor := VisitorSafeFarmSnapshot(agg.Snapshot())
	if visitor.GuardDog.ActiveDog != DogShepherd || visitor.GuardDog.BowlEmptyAt != 12_345 {
		t.Fatalf("guard dog = %#v, want active shepherd with bowl expiry", visitor.GuardDog)
	}
	payload, err := json.Marshal(visitor)
	if err != nil {
		t.Fatalf("marshal visitor snapshot: %v", err)
	}
	if strings.Contains(string(payload), "intercepts") || strings.Contains(string(payload), "dog_level") {
		t.Fatalf("visitor snapshot leaked private dog data: %s", payload)
	}
}
