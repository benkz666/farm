...
func TestBuyNotEnoughCoin(t *testing.T) {
	...
	result := agg.Buy(BuyReq{ItemID: 1, Quantity: 1})
	if result.Err != errcode.NotEnoughCoin {
		t.Fatalf("Err = %d, want %d", result.Err, errcode.NotEnoughCoin)
	}
	if agg.Coin != 100 {
...
func TestBuyCropLocked(t *testing.T) {
	...
	if result.Err != errcode.CropLocked {
		t.Fatalf("Err = %d, want %d", result.Err, errcode.CropLocked)
	}
}
...
func TestBuyHiddenCropRejected(t *testing.T) {
	...
	if result.Err != errcode.ItemNotFound {
...
func TestSellNotEnoughItem(t *testing.T) {
	...
	if result.Err != errcode.NotEnoughItem {
		t.Fatalf("Err = %d, want %d", result.Err, errcode.NotEnoughItem)
	}
	if agg.Coin != before {
		t.Fatalf("Coin mutated on failure: %d", agg.Coin)
	}
