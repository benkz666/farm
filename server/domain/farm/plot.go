...
const (
	...
	StateMature                 // 成熟：健康/产量冻结，可收获/偷取并清理已有负面状态
	StateResidue                // 待清理：末季收获完成，残留植株待清理
	StateWithered               // 枯萎：兼容历史数据；普通作物不会因成熟后未照料进入
	stateCount
)
...
func plotActionStateError(state uint8, action PlotActionKind) errcode.Code {
	switch action {
	case Till:
		return errcode.PlotNotWasteland
	case Clear:
		return errcode.PlotNotCleanable
	case Plant:
		return errcode.PlotNotTilled
	case Water, Weed, Pest, Fertilize:
		return errcode.PlotNotGrowing
	case Harvest, Steal:
		if state == StateWithered {
			return errcode.PlotWithered
		}
		return errcode.PlotNotMature
	default:
...
type Plot struct {
	...
	FinalYield  uint16 `json:"final_yield"`  // 成熟瞬间按健康度固化的实际产量
