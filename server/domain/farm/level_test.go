...
func TestRecalcLevelMatchesClientRule(t *testing.T) {
	...
	if gameconfig.ExpPerLevel != 200 {
		t.Fatalf("ExpPerLevel=%d, want 200（与客户端 EXP_PER_LEVEL 一致）", gameconfig.ExpPerLevel)
	}
}
...
func TestAppleUnlockViaAccumulatedExp(t *testing.T) {
	...
	if locked.Err != errcode.CropLocked {
