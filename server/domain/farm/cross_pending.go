...
func (a *Aggregate) ReserveCross(reservation CrossReservation, now int64) (CrossReservation, errcode.Code) {
	if a == nil {
		return reservation, errcode.Internal
	}
	...
	if len(a.CrossPending) >= maxCrossPending {
		return reservation, errcode.RateLimited
	}

	reservation.ReservedAt = now
	if reservation.Steal {
		if code := a.FreezeStealCompensation(reservation.FrozenCoin); code != errcode.OK {
