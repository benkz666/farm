...
func TestAdvanceKeepsMatureCropHealthAndYieldFrozen(t *testing.T) {
	p := Plot{
		State:          StateMature,
		CropID:         1,
		SeasonDuration: testSeasonDuration,
		MatureAt:       testSeasonStart + testSeasonDuration,
		LastSettleAt:   testSeasonStart + testSeasonDuration,
		LastWaterAt:    testSeasonStart + testSeasonDuration,
		FinalYield:     14,
	}
	cfg := advanceConfig(t, p.CropID)
	cfg.WeedThreshold = 0
	...
	before := p
	Advance(&p, p.MatureAt+3_000, cfg)
	if !reflect.DeepEqual(p, before) {
		t.Fatalf("mature plot changed over time:\n got %#v\nwant %#v", p, before)
	}
}
