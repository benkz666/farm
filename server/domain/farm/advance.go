...
func NewAdvanceConfig(crop gameconfig.CropConf) AdvanceConfig {
	return AdvanceConfig{
		BaseYield:             crop.Yield,
		DryWeight:             int64(gameconfig.WeightDry * float64(healthPointsPerFullPenalty)),
		WeedWeight:            int64(gameconfig.WeightWeed * float64(healthPointsPerFullPenalty)),
		PestWeight:            int64(gameconfig.WeightPest * float64(healthPointsPerFullPenalty)),
		WaterSpanNumerator:    int64(gameconfig.WaterSpanRatio * float64(healthPointsPerFullPenalty)),
		WaterSpanDenominator:  healthPointsPerFullPenalty,
		RiskWindowNumerator:   int64(gameconfig.RiskWindowRatio * float64(healthPointsPerFullPenalty)),
		RiskWindowDenominator: healthPointsPerFullPenalty,
		WeedThreshold:         gameconfig.WeedHazardThreshold,
		PestThreshold:         gameconfig.PestHazardThreshold,
		RiskWindows:           gameconfig.RiskWindowsPerSeason,
	}
}
...
func Advance(p *Plot, now int64, cfg AdvanceConfig) {
	...
	if p.State != StateGrowing {
		// 经典 QQ 农场规则：成熟瞬间冻结健康度和产量；负面字段保留供玩家显示/清理，
		// 但负面状态不再随时间扣减，也不会因长期未收获而枯萎。
		return
	}
...
// settleTo 把生长期健康度结算推进到 to。成熟点由 Advance 截断，成熟后不再调用。
func settleTo(p *Plot, to int64, cfg AdvanceConfig) {
	...
	increment := cfg.DryWeight*dryMs + cfg.WeedWeight*weedMs + cfg.PestWeight*pestMs
	maxAccrued := healthPointsPerFullPenalty * p.SeasonDuration
	if p.AccruedWeighted >= maxAccrued || increment >= maxAccrued-p.AccruedWeighted {
		p.AccruedWeighted = maxAccrued
	} else {
		p.AccruedWeighted += increment
	}
	p.LastSettleAt = to
}
...
func scanHazard(p *Plot, from, to int64, since *int64, nextWin *uint8, kind uint8, cfg AdvanceConfig) int64 {
	...
	if windows == 0 {
		windows = gameconfig.RiskWindowsPerSeason
	}
	if *since == 0 {
...
func hazardHit(cfg AdvanceConfig, p *Plot, kind, window uint8) bool {
	...
	return hazardRoll(cfg, p, kind, window)%gameconfig.HazardRollMod < uint64(threshold)
}
