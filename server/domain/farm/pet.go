...
const (
	...
	DogBowlCapacityGrams uint32 = 200
...
func (a *Aggregate) PetActivate(dog DogType, now int64) ActionResult {
	return a.PetActivateWithProfile(dog, now, gameconfig.TimeProfileDemo)
}
...
func (a *Aggregate) PetFeed(req PetFeedReq, now int64) ActionResult {
	return a.PetFeedWithProfile(req, now, gameconfig.TimeProfileDemo)
}
...
func (a *Aggregate) PetFeedWithProfile(req PetFeedReq, now int64, timeProfile string) ActionResult {
	...
	if remaining >= DogBowlCapacityGrams {
		return ActionResult{Err: errcode.BowlFull}
	}
	add := req.Grams
	...
	key := DogFoodItem()
	if a.Items[key] < add {
		return ActionResult{Err: errcode.NoDogFood}
	}
	a.Items[key] -= add
	if a.Items[key] == 0 {
		delete(a.Items, key)
...
func (a *Aggregate) FreezeStealCompensation(amount int64) errcode.Code {
	if a == nil || amount <= 0 {
		return errcode.BadRequest
	}
	if a.Coin < amount {
		return errcode.StealNoAfford
	}
	a.Coin -= amount
	a.FarmSeq++
	return errcode.OK
...
func dogMsPerGram(dog DogType, timeProfile string) int64 {
	hourMs := gameconfig.HourMs(timeProfile)
	if hourMs <= 0 {
		hourMs = gameconfig.HourMs(gameconfig.TimeProfileDemo)
	}
	switch dog {
	case DogMutt:
		return hourMs / 4
