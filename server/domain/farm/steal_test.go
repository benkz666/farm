...
func TestApplyStealCapsAtFortyPercentAndTruncatesRandomAmount(t *testing.T) {
	...
	if exhausted.Err != errcode.StealQuotaExhausted {
		t.Fatalf("quota exhausted Err = %d, want %d", exhausted.Err, errcode.StealQuotaExhausted)
	}
}
...
func TestApplyStealAllowsVisitorOncePerHarvestRound(t *testing.T) {
	...
	if first.Err != errcode.OK {
		t.Fatalf("first Err = %d, want OK", first.Err)
	}
	repeated := owner.ApplySteal(action, actionNow)
	if repeated.Err != errcode.StealAlreadyDone {
		t.Fatalf("repeated Err = %d, want %d", repeated.Err, errcode.StealAlreadyDone)
	}
...
func TestApplyStealDoesNotCollideDistinctUint64Visitors(t *testing.T) {
	...
	if first.Err != errcode.OK || second.Err != errcode.OK {
...
func TestApplyStealAfterOwnerHarvestReturnsHarvestedByOwner(t *testing.T) {
	...
	if result.Err != errcode.HarvestedByOwner {
		t.Fatalf("Err = %d, want %d", result.Err, errcode.HarvestedByOwner)
	}
}
