...
func runSteal(baseURL string) error {
	...
	for i := 0; i < 4; i++ {
		v, err := openSmokePlayer(baseURL, routes, owner.farm)
		if err != nil {
	...
	fmt.Printf("smoke steal: owner uid=%d farm=%s; visitors=%d on %s\n",
		owner.login.UID, owner.farm, len(visitors), owner.farm)
	...
	for i, v := range visitors {
		...
		case errcode.StealQuotaExhausted:
	...
	if !sawQuotaExhausted {
		extra, err := openSmokePlayer(baseURL, routes, owner.farm)
		if err != nil {
		...
		case errcode.OK:
			...
			extra2, err := openSmokePlayer(baseURL, routes, owner.farm)
			if err != nil {
			...
			if env2.Err != errcode.StealQuotaExhausted {
				return fmt.Errorf("quota exhaust2 err = %d, want %d (stolen=%d cap=%d)",
					env2.Err, errcode.StealQuotaExhausted, stolenTotal, quotaCap)
			}
			sawQuotaExhausted = true
		default:
	...
	if harvestEnv.Err != errcode.HarvestedByOwner {
		return fmt.Errorf("harvest race err = %d, want %d", harvestEnv.Err, errcode.HarvestedByOwner)
	}
	...
	broke, err := openSmokePlayer(baseURL, routes, owner.farm)
	if err != nil {
	...
	spend := gameconfig.InitialCoin - (compensation - 1)
	if spend < 1 {
		spend = 1
	}
	...
	if noAffordEnv.Err != errcode.StealNoAfford {
		return fmt.Errorf("no-afford err = %d, want %d", noAffordEnv.Err, errcode.StealNoAfford)
	}
	...
	for attempt := 0; attempt < stealMaxIntercept && !intercepted; attempt++ {
		if attempt > 0 {
			plotIdx = (plotIdx + 1) % uint32(gameconfig.InitialUnlockedPlots)
			if err := ownerResetAndMature(owner, baseURL, plotIdx); err != nil {
...
func openSmokePlayer(baseURL string, routes *sharding.RouteTable, wantFarm string) (*smokePlayer, error) {
...
func ownerEarnForDog(owner *smokePlayer, baseURL string, crop gameconfig.CropConf) error {
	...
	var coin int64 = -1
	for plot := uint32(0); plot < uint32(gameconfig.InitialUnlockedPlots); plot++ {
	...
	for attempt := 0; attempt < 24; attempt++ {
		...
		plot := uint32(attempt % int(gameconfig.InitialUnlockedPlots))
...
func mustOwnerAction(owner *smokePlayer, cmd uint32, payload map[string]any) (actionPayload, error) {
	...
	if env.Err != errcode.OK {
		return actionPayload{}, fmt.Errorf("err = %d, want %d", env.Err, errcode.OK)
	}
