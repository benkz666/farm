...
func TestCodexProgressRewardMailIsIdempotentAndProtectedFromClear(t *testing.T) {
	...
	// crop 8 会把位图首字节置为 0x80，覆盖批量 SQL 必须按二进制而非
	// utf8mb4 字符串传递 codex_bitmap 的回归场景。
	agg.CodexHarvests[8] = 1
	...
	if got := reloaded.CodexHarvests[8]; got != 1 {
		t.Fatalf("binary bitmap harvest count = %d, want 1", got)
	}
...
func assertFreshFarm(t *testing.T, agg *farm.Aggregate, uid uint64, username string) {
	...
	if agg.Coin != gameconfig.InitialCoin {
		t.Fatalf("want coin=%d got %d", gameconfig.InitialCoin, agg.Coin)
	}
	if agg.UnlockedPlots != gameconfig.InitialUnlockedPlots {
		t.Fatalf("want unlocked_plots=%d got %d", gameconfig.InitialUnlockedPlots, agg.UnlockedPlots)
	}
	if len(agg.Plots) != gameconfig.MaxPlots {
		t.Fatalf("want %d plots got %d", gameconfig.MaxPlots, len(agg.Plots))
	}
	if agg.Items == nil {
