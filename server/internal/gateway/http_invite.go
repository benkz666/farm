...
func (g *Gateway) inviteLanding(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, inviteLandingResponse{OK: errCode == pkgerr.OK, Err: errCode})
}
...
func (g *Gateway) acceptInviteForUser(ctx context.Context, uid uint64, token string) pkgerr.Code {
	if g.friends == nil {
	...
	inviterUID, code := social.ParseInvite(token, g.inviteSecret, g.Now())
	if code != pkgerr.OK {
		return code
	}
	if inviterUID == 0 {
		return pkgerr.InviteInvalid
	}
