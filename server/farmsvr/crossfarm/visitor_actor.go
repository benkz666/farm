...
func ReserveVisitor(aggregate *farm.Aggregate, reservation VisitorReservation, now int64) errcode.Code {
	...
	if reservation.Action.Kind == Steal {
		crop, ok := gameconfig.CropByID(reservation.Action.CropID)
		if !ok {
			return errcode.BadRequest
		}
		...
		entry.Steal = true
		entry.FrozenCoin = gameconfig.StealCompensation(crop)
	} else {
...
// 天然幂等；返回 errcode.Timeout 表示该预占已被超时回滚，收益不再补发。
func SettleVisitor(
	aggregate *farm.Aggregate,
	result CrossResult,
	now int64,
) (VisitorReward, *farm.PlayerDelta, errcode.Code) {
...
func settleVisitorSteal(
	...
	case errcode.StealIntercepted:
		...
		// 与主人侧收到的是同一个数（都由 gameconfig.StealCompensation 算出）。
...
func settleVisitorMaintenance(
	...
	if !reservation.Rewarded {
		...
		return reward, nil, errcode.OK
	}
	reward.ExpGained, reward.CoinGained = exp, coin

	delta := aggregate.PlayerDelta()
	return reward, &delta, errcode.OK
}
