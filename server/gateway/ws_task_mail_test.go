...
func TestRemoteTaskMailRoutesThroughFarmRPCWithoutLocalStore(t *testing.T) {
	t.Parallel()
	...
	local := newTaskMailStoreStub()
	client := &farmRPCStub{responses: []farmrpc.CommandResponse{
		{
			Err: errcode.OK,
			Payload: marshalPayload(farmrpc.TaskListResponse{
				Tasks:   []store.Task{{ID: 1, Title: "remote", Progress: 1, Target: 1}},
				ResetAt: 123,
			}),
		},
		{
			Err:     errcode.OK,
			Payload: marshalPayload(farmrpc.MailMutationResponse{Affected: 2}),
		},
	}}
	gateway := New(
		authStub{},
		...
		WithTaskMailStore(local),
	)
	connection := &wsConnection{uid: 42, authed: true}

	taskList := gateway.handleWSRequest(connection, Envelope{
		Cmd:     CommandTaskList,
		Payload: emptyPayload,
	})
	if taskList.Err != errcode.OK {
		t.Fatalf("TaskList err = %d, want OK", taskList.Err)
	}
	if len(local.progressCalls) != 0 || local.lastMailUID != 0 {
		t.Fatalf("local taskMail touched: progress=%#v mailUID=%d", local.progressCalls, local.lastMailUID)
	}

	mailRead := gateway.handleWSRequest(connection, Envelope{
		Cmd:     CommandMailRead,
		Payload: json.RawMessage(`{"all":true}`),
	})
	if mailRead.Err != errcode.OK {
		t.Fatalf("MailRead err = %d, want OK", mailRead.Err)
	}
	if local.lastMailUID != 0 {
		t.Fatalf("local taskMail MailRead uid = %d, want untouched", local.lastMailUID)
	}

	if len(client.calls) != 2 {
		t.Fatalf("farmRPC calls = %d, want 2", len(client.calls))
	}
	if client.calls[0].request.Operation != farmrpc.OperationTaskList ||
		client.calls[1].request.Operation != farmrpc.OperationMailRead {
		t.Fatalf("farmRPC ops = %q then %q, want task_list then mail_read",
			client.calls[0].request.Operation, client.calls[1].request.Operation)
	}
}
...
func TestCodexListReturnsAuthoritativePerCropPlaques(t *testing.T) {
	...
	if payload.Total != gameconfig.CropCount || len(payload.Entries) != 2 {
...
func TestDailyLoginAndTaskRewardsAreDirectAndIdempotent(t *testing.T) {
	...
	if secondDaily.Err != errcode.DuplicateOK {
		t.Fatalf("repeated ClaimDailyLogin err = %d, want %d", secondDaily.Err, errcode.DuplicateOK)
	}
	...
	if tasks.Err != errcode.OK || len(taskList.Tasks) != 4 {
		t.Fatalf("TaskList = %#v, payload = %#v", tasks, taskList)
	}
	if taskList.ResetAt != gameconfig.NextLocalDayResetMs(now) {
		t.Fatalf("TaskList reset_at = %d, want %d", taskList.ResetAt, gameconfig.NextLocalDayResetMs(now))
	}
	...
	if duplicateDailyTask.Err != errcode.TaskAlreadyClaimed {
		t.Fatalf("TaskClaim daily after ClaimDailyLogin err = %d, want %d", duplicateDailyTask.Err, errcode.TaskAlreadyClaimed)
	}
	...
	if mails.Err != errcode.OK || len(mailList.Mails) != 0 {
	...
	if duplicateTaskClaim.Err != errcode.TaskAlreadyClaimed {
		t.Fatalf("repeated TaskClaim err = %d, want %d", duplicateTaskClaim.Err, errcode.TaskAlreadyClaimed)
	}

	if aggregate.Coin != gameconfig.InitialCoin+dailyReward.Coin+taskReward.Coin {
		t.Fatalf("online actor coin = %d, want %d", aggregate.Coin, gameconfig.InitialCoin+dailyReward.Coin+taskReward.Coin)
	}
	...
	if missingMail.Err != errcode.MailNotFound {
		t.Fatalf("missing MailClaim err = %d, want %d", missingMail.Err, errcode.MailNotFound)
	}
	notice := storage.addMail("system notice", 0)
	noAttachment := gateway.handleWSRequest(connection, Envelope{
		Cmd:     CommandMailClaim,
		Payload: marshalPayload(mailClaimRequest{MailID: clientjson.Uint64(notice.ID)}),
	})
	if noAttachment.Err != errcode.MailNoAttachment {
		t.Fatalf("empty MailClaim err = %d, want %d", noAttachment.Err, errcode.MailNoAttachment)
	}
}
...
func TestSuccessfulPlantAdvancesDailyTaskProgress(t *testing.T) {
	...
	want := taskProgressCall{uid: 42, dayKey: gameconfig.LocalDayKey(now), taskID: store.TaskPlantID, amount: 1}
...
func TestSuccessfulHarvestAdvancesTaskAndPublishesTaskNotify(t *testing.T) {
	...
	aggregate.Plots[0] = farm.Plot{
		...
		CropID:         1,
		SeasonDuration: gameconfig.HourMs(gameconfig.TimeProfileDemo),
		MatureAt:       1,
		FinalYield:     16,
	}
	gateway := New(
	...
	want := taskProgressCall{uid: 42, dayKey: gameconfig.LocalDayKey(gateway.Now()), taskID: store.TaskHarvestID, amount: 1}
	if len(storage.progressCalls) != 1 || storage.progressCalls[0] != want {
		t.Fatalf("progress calls = %#v, want %#v", storage.progressCalls, want)
	}
	select {
...
func TestFriendEnterAdvancesDailyVisitTask(t *testing.T) {
	...
	want := taskProgressCall{uid: visitorUID, dayKey: gameconfig.LocalDayKey(now), taskID: store.TaskVisitID, amount: 1}
	if len(storage.progressCalls) != 1 || storage.progressCalls[0] != want {
		t.Fatalf("progress calls = %#v, want %#v", storage.progressCalls, want)
	}
}
