...
func TestPetEmptyBowlCannotInterceptAndFedDogUsesConfiguredRate(t *testing.T) {
	...
	if result := agg.PetActivate(DogMutt, 1_000); result.Err != pkgerr.OK {
...
func TestAllDogBreedsRespectUnlockPriceAndIndependentOwnership(t *testing.T) {
	...
	if result := agg.Buy(BuyReq{ItemID: DogShepherdShopItemID, Quantity: 1}); result.Err != pkgerr.DogLocked {
		t.Fatalf("locked shepherd buy Err = %d, want %d", result.Err, pkgerr.DogLocked)
	}
	...
	agg.Level = 10
	if result := agg.Buy(BuyReq{ItemID: DogShepherdShopItemID, Quantity: 1}); result.Err != pkgerr.OK {
	...
	agg.Level = 20
	if result := agg.Buy(BuyReq{ItemID: DogMastiffShopItemID, Quantity: 1}); result.Err != pkgerr.OK {
	...
	if result := agg.Buy(BuyReq{ItemID: DogMastiffShopItemID, Quantity: 1}); result.Err != pkgerr.DogAlreadyOwned {
		t.Fatalf("duplicate mastiff buy Err = %d, want %d", result.Err, pkgerr.DogAlreadyOwned)
	}
}
...
func TestSwitchingBreedPreservesFoodAndAppliesNewConsumptionRate(t *testing.T) {
	...
	if result := agg.PetFeed(PetFeedReq{Grams: 10}, 1_000); result.Err != pkgerr.OK {
	...
	if result := agg.PetActivate(DogShepherd, 4_000); result.Err != pkgerr.OK {
	...
	if result := agg.PetActivate(DogMastiff, 5_200); result.Err != pkgerr.OK {
