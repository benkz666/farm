...
func TestHTTPDeltaPusherRetriesTemporaryGatewayFailure(t *testing.T) {
	t.Parallel()
	...
	if err := pusher.PushBatch(t.Context(), "gateway-0", PushBatch{
		ConnIDs:  []uint64{7},
		Envelope: []byte(`{"cmd":9000,"client_seq":0,"err":0,"payload":{"owner_uid":"42","farm_seq":"3"}}`),
	}); err != nil {
		t.Fatalf("PushBatch: %v", err)
	}
...
func TestHTTPDeltaPusherDoesNotRetryPermanentGatewayFailure(t *testing.T) {
	t.Parallel()
	...
	pusher := NewHTTPDeltaPusher(map[string]string{"gateway-0": server.URL}, "internal-token")
	err := pusher.PushBatch(t.Context(), "gateway-0", PushBatch{
		ConnIDs:  []uint64{7},
		Envelope: []byte(`{"cmd":9000,"client_seq":0,"err":0,"payload":{"owner_uid":"42","farm_seq":"3"}}`),
	})
	if err == nil {
		t.Fatal("PushBatch error = nil, want permanent HTTP 400 error")
	}
