...
func (p *HTTPDeltaPusher) PushBatch(ctx context.Context, gatewayID string, batch PushBatch) error {
	...
	for attempt := 1; attempt <= deltaPushMaxAttempts; attempt++ {
		request, requestErr := http.NewRequestWithContext(
			ctx,
			http.MethodPost,
			endpoint+deltaPushBatchPath,
			bytes.NewReader(body),
		)
		if requestErr != nil {
			return fmt.Errorf("farmrpc: build Delta push batch: %w", requestErr)
		}
		request.Header.Set("Authorization", "Bearer "+p.token)
		request.Header.Set("Content-Type", "application/json")

		response, pushErr := p.client.Do(request)
		if pushErr == nil {
			response.Body.Close()
			if response.StatusCode == http.StatusNoContent {
				return nil
			}
			lastErr = fmt.Errorf("farmrpc: Delta push batch returned HTTP %d", response.StatusCode)
			// 参数或鉴权错误重试也不会恢复，只重试限流和服务端临时故障。
