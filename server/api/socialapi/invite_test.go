...
func TestIssueAndParseInvite(t *testing.T) {
	...
	token, err := social.IssueInvite(inviter, now, []byte(testSecret))
	if err != nil {
	...
	got, code := social.ParseInvite(token, []byte(testSecret), now)
	if code != pkgerr.OK {
...
func TestParseInviteTamperedSignature(t *testing.T) {
	...
	if _, code := social.ParseInvite(tampered, []byte(testSecret), now); code != pkgerr.InviteInvalid {
...
func TestParseInviteWrongSecret(t *testing.T) {
	...
	if _, code := social.ParseInvite(token, []byte("different-secret"), now); code != pkgerr.InviteInvalid {
...
func TestParseInviteExpired(t *testing.T) {
	...
	token, err := social.IssueInvite(inviter, issueTime, []byte(testSecret))
	if err != nil {
		t.Fatalf("IssueInvite: %v", err)
	}
	...
	if _, code := social.ParseInvite(token, []byte(testSecret), afterExpiry); code != pkgerr.InviteExpired {
...
func TestParseInviteJustBeforeExpiry(t *testing.T) {
	...
	got, code := social.ParseInvite(token, []byte(testSecret), atExpiry)
	if code != pkgerr.OK {
...
func TestParseInviteMalformed(t *testing.T) {
	...
	for _, tok := range cases {
		if _, code := social.ParseInvite(tok, []byte(testSecret), 1); code != pkgerr.InviteInvalid {
...
func TestIssueInviteRejectsEmptySecret(t *testing.T) {
	if _, err := social.IssueInvite(1, 1, nil); err == nil {
		t.Fatal("IssueInvite with nil secret should error")
	}
	if _, err := social.IssueInvite(1, 1, []byte{}); err == nil {
